...
describe("API smoke test", () => {
    ...
    it("GET request with RESTAURANT query", () => {
        ...
        cy.request(`http://localhost:5000/api/pizzas?restaurant=${input}`).then(response => {
            expect(response.status).to.eq(200);
    ...
    it("GET request with SCORE query", () => {
        const score = 5;
        cy.request(`http://localhost:5000/api/pizzas?score=${score}`).then(response => {
            expect(response.status).to.eq(200);
            response.body.forEach(element => {
    ...
    it("GET request with PIZZA query", () => {
        ...
        cy.request(`http://localhost:5000/api/pizzas?pizza=${pizza}`).then(response => {
            expect(response.status).to.eq(200);
            response.body.forEach(element => {
    ...
    it("GET request with STYLE query", () => {
        const style = "Classic";
        cy.request(`http://localhost:5000/api/pizzas?style=${style}`).then(response => {
            expect(response.status).to.eq(200);
            response.body.forEach(element => {
    ...
    it("POST request test", () => {
        cy.request("POST", "http://localhost:5000/api/pizzas", postPayload).then(response => {
...
describe("UI Smoke Test", function () {
    it("Application opens", function () {
        cy.xpath("//h1").should("be.visible").and("contain", "Welcome to Pizza Club");
    });
    ...
    it("Add Pizza form should be visible", () => {
        cy.xpath("//button[@data-test-id='add-pizza-btn']").click();
...
describe("Add Pizza Test", () => {
    it("Fill out form", () => {
        cy.xpath("//form[@id='add-pizza-form']").scrollIntoView().xpath("./input[@data-test-id='restaurant']").type("Test Restaurant " + rest).should("have.value", "Test Restaurant " + rest).xpath("//input[@data-test-id='pizza']").type("Test pizza " + rest).should("have.value", "Test pizza " + rest).xpath("//input[@data-test-id='description']").type("Test description " + rest).should("have.value", "Test description " + rest).xpath("//input[@data-test-id='style']").type("Test Style " + rest).should("have.value", "Test Style " + rest).xpath("//select[@data-test-id='score']").select("4").should("have.value", "4");
    });
});
