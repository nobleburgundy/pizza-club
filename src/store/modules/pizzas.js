...
const actions = {
    ...
    async addPizza({
        commit
    }, restaurant, pizza, description, style, score) {
    ...
    async deletePizza({
        ...
        const response = await axios.delete(`http://localhost:5000/api/pizzas/${id}`);
    ...
    filterPizzas(
        ...
        const score = parseInt(e.target.options[e.target.options.selectedIndex].innerText) || 0;
        if (score === 0) {
            dispatch("fetchPizzas");
        } else {
            dispatch("fetchPizzas").then(() => commit("filterPizzas", score)).catch(err => console.log(err.message));
        }
    },
    async updatePizza({
        commit
    }, updatedPizza) {
        const response = await axios.put(`http://localhost:5000/api/pizzas/${
            updatedPizza._id
        }`, updatedPizza);
...
const mutations = {
    ...
    removePizza: (state, id) => (state.pizzas = state.pizzas.filter(pizza => pizza._id !== id)),
    filterPizzas: (state, score) => (state.pizzas = state.pizzas.filter(pizza => pizza.score == score)),
    updatePizza: (state, updatedPizza) => {
        const index = state.pizzas.findIndex(pizza => pizza.id === updatedPizza._id);
        if (index !== -1) {
